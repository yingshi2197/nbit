...
!function ($) {
    ...
    var calculateObjectValue = function (self, name, args, defaultValue) {
        if (typeof name === 'string') {
        	// modify by kent   增加数据字典的列格式化方法     	
        	if(name.indexOf("dictCode:")==0) {
        		var dictCodes=name.replace("dictCode:","");
        	...
        	if(name.indexOf("date:")==0) {
        		var format=name.replace("date:","");
        		return tools.parseDate(args[0],format);
        	}
        ...
        if (typeof name === 'function') {
            return name.apply(self, args,args[1]);
        }
        return defaultValue;
    };
    ...
    BootstrapTable.LOCALES['zh-CN'] = {
            ...
            formatShowingRows: function (pageFrom, pageTo, totalRows) {
                //return sprintf('Showing %s to %s of %s rows', pageFrom, pageTo, totalRows);
                //return sprintf('显示 %s to %s 总共 %s 行', pageFrom, pageTo, totalRows);
                return sprintf('显示 %s to %s', pageFrom, pageTo);
            },
            formatSearch: function () {
                return 'Search';
            },
    ...
    BootstrapTable.COLUMN_DEFAULTS = {
        ...
        width: undefined,
        sortable: false,
        formatterParam:null,  // 格式化列时需要传递的参数
        order: 'asc', // asc, desc
        visible: true,
        switchable: true,
        clickToSelect: true,
    ...
    BootstrapTable.prototype.initHeader = function () {
        ...
        $.each(this.options.columns, function (i, column) {
            ...
            style = sprintf('vertical-align: %s; ','middle' /*column.valign*/);
    ...
    BootstrapTable.prototype.onSort = function (event) {
        ...
        var index_=$this.data('index')|| $this.data('field');
        this.$header.add(this.$header_).find('span.order').remove();

        if (this.options.sortName === index_) {
            this.options.sortOrder = this.options.sortOrder === 'asc' ? 'desc' : 'asc';
        } else {
            this.options.sortName = index_;
    ...
    BootstrapTable.prototype.initPagination = function () {
        ...
        this.tempTotalPages=to+5;
        for (i = from; i <= to; i++) {
            html.push('<li class="page-number' + (i === this.options.pageNumber ? ' active' : '') + '">',
                '<a href="javascript:void(0)">', i, '</a>',
                '</li>');
        }
       /* html.push('<li class="page-number' + ' active'  + '">',
                '<a href="javascript:void(0)">', this.options.pageNumber, '</a>',
                '</li>');*/
    ...
    BootstrapTable.prototype.onPageLast = function (event) {
    	// this.options.pageNumber = this.totalPages;  
        this.options.pageNumber = this.tempTotalPages;
        this.updatePagination(event);
    };
        ...
        for (var i = this.pageFrom - 1; i < this.pageTo; i++) {
            ...
            $.each(this.header.fields, function (j, field) {
                var text = '',
                	...
                	//alert(tools.objToJson(column));
                style = sprintf('style="%s"', csses.concat(that.header.styles[j]).join('; '));
                item["formatterParam"]=column["formatterParam"];
                value = calculateObjectValue(that.header,
                ...
                if (column.checkbox || column.radio) {
                    ...
                    text = [that.options.cardView ?
                        '<div class="card-view">' : '<td class="bs-checkbox"  style="vertical-align:middle" >',
                        '<input' +
                            sprintf(' data-index="%s"', i) +
                    ...
                    text = that.options.cardView ?
                        ['<div class="card-view">',
                            ...
                            '</div>'].join('') :
                        [sprintf('<td%s %s %s %s title="'+(that.header.formatters[j] ? "" :escapeHTML(value))+'">', id_, class_, style, data_),
                         (that.header.formatters[j] ? value :escapeHTML(tools.cutstr(value, column.filterLen||12))),
                            '</td>'].join('');
    ...
    BootstrapTable.prototype.initServer = function (silent, query) {
        ...
        var searchDiv=this.options.searchDiv;
		 ...
		 query["condition"]=tools.objToJson(search);
		
		 //alert(tools.objToJson(query));
        ...
        $.extend(true,data, {"globalParam":this.options.param||''});
    ...
    BootstrapTable.prototype.load = function (data) {
        ...
        if(this.options.completeLoad) {
        	this.options.completeLoad(this.options);
        }
    };
    ...
    BootstrapTable.prototype.getSelections = function () {
        ...
        var data_=$.grep(this.data, function (row) {
            return row[that.header.stateField];
        });
        
        var radio=that.isRadio();
        if(radio) {
        	if(data_&&data_.length>0)
        		return data_;
        }
        
        if(that["selectData"]&&that["selectData"].length>0) {
        	for(var i=0;i<that["selectData"].length;i++) {
        		var obj=that["selectData"][i];
        		if(!tools.contain(data_, obj))
        			data_.push(obj);
        	}
        }
        return data_;
    ...
    // add by kent 2015-05-13
    BootstrapTable.prototype.isRadio = function () {
        var that = this;
        
        var columns=that.options.columns;
        if(columns) {
        	return columns[0].radio;
        }
    };
    
    BootstrapTable.prototype.addSelections = function (data) {
        var that = this;
        if(!this["selectData"])
        	this["selectData"]=data;
        //alert(this["selectData"].length);
    };
    
    BootstrapTable.prototype.resetSelections = function () {
        var that = this;
        this["selectData"]=[];
    };
    
    BootstrapTable.prototype.replaceSelections = function (row) {
        var that = this;
        ...
        if(radio) {
        	that["selectData"]=[];
        	that["selectData"].push(row);
        	return that["selectData"];
        }
        
       
    	var index=tools.getIndex(that["selectData"], row,["id"]);
		if(index!=-1)
			that["selectData"][index]=row;
		else
			that["selectData"].push(row);
		//alert(that["selectData"].length);
    };
    
    BootstrapTable.prototype.removeSelections = function (row) {
        var that = this;
        if(!that["selectData"])
        	that["selectData"]=[];
        if(that["selectData"]&&that["selectData"].length>0) {
    ...
    BootstrapTable.prototype.refresh = function (params) {
        ...
        var data__=this.options.data;
        var operateType=params["operateType"];
        //alert(this.options.pageNumber+"|"+operateType);
        if(data__&&data__.length==1&&"remove"==operateType&&this.options.pageNumber>1) {
    ...
    var allowedMethods = [
        'getOptions',
        'getSelections', 'getData','addSelections','replaceSelections','removeSelections','resetSelections',

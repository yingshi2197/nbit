...
var lhgdialog = function( config )
{
	...
	_count++;
	lhgdialog.parentDialog=config.p;
...
lhgdialog.fn = lhgdialog.prototype =
{
	...
	close: function(_name)
	{
		...
		if( that.iframe )
		{
			lhgdialog.parentDialog=that.config.p;
			if( typeof  fn === 'function' && fn.call(that, that.iframe.contentWindow,that.config.p) === false ) {
				 return that;
			}
		...
		if(_name=="cancel"||(!that.config)||(that.config&&that.config.icon!="confirm.gif"))
			that.unlock();
		...
		//lhgdialog.focus =lhgdialog.parentDialog;
		that._removeEvent();
		...
		//alert(tiger);
		if(arr.length>0) {
			if(arr[arr.length-1].content.tiger===false) {
				//alert(arr[arr.length-1].content.tiger);
				arr[arr.length-1].content.tiger=true;
			}
	...
	_getDOM: function()
	{
		...
		for( ; i < len; i ++ )
		{
			name = els[i].className.split(" ")[0].split('ui_')[1];
...
lhgdialog.setting =
{
	...
	max: true,                  // 是否显示最大化按钮
	min: true,                  // 是否显示最小化按钮
	zIndex: 7976,				// 对话框叠加高度值(重要：此值不能超过浏览器最大限制)
